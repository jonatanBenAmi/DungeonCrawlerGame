...
public class Player_Charactor : Charactor_Class 
{
		...
		if(File.Exists(path + "/profileSave.fzf")){
			...
			while((s = fileLoaded.ReadLine()) != null){
				...
				if(getLine[0] == "Player Lives"){
					lives = int.Parse(getLine[1]);
				}
				if(getLine[0] == "Player Defence"){
					defence = float.Parse(getLine[1].ToString(), System.Globalization.CultureInfo.InvariantCulture);
				}
			}
			// Close the file
			fileLoaded.Close();
	...
	public string saveFileFormat(){
		...
		string data = "";
		// adds lines to it
		data += "Player Position X=" + transform.position.x + "\n";
		data += "Player Position Y=" + transform.position.y + "\n";
		data += "Player Position Z=" + transform.position.z + "\n";
